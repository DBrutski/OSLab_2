...
void init_pages_offset(memory_pager *self, size_t page_size) {
    ...
    self->page_offset_bits = MaxDivPow2(casted_page_size);
}
...
void
write_page(memory_pager *self, page current_page, size_t page_offset, char *buffer, size_t buffer_size) {
...
void
read_page(memory_pager *self, page current_page, size_t page_offset, char *buffer, size_t buffer_size) {
